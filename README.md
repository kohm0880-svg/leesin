...
- Sample Size N = 같은 Experiment Goal과 호환 Axis 구성을 가진 저장 cluster 개수이며, CSV row 수가 아닙니다.
...
- Coverage C = b / a 입니다. 여기서 a는 Domain Range를 Resolution으로 나눈 전체 bin 수이고, b는 row-level observation이 점유한 bin 수입니다.
- Equitability E = -Σ p_i ln(p_i) / ln(b) 입니다. 점유된 row-level bin들 사이 observation count가 얼마나 균등한지 나타냅니다.
- Heterogeneity는 cluster 대표 vector 기준의 D² = (x - center)^T Σ^-1 (x - center)로 계산합니다.
- p-value = 1 - F_{χ²,p}(D²), heterogeneity = 1 - p-value ≈ F_{χ²,p}(D²) 입니다.
- final confidence = (Z * C * E)^(1/3) * w_eff 입니다.
- w_eff는 `spatial_rank`에서 0.5, `mahalanobis`/`sscm`에서 min(1, p(p+2) / b2p)를 사용합니다.
...
- `binOccupancyHash`: duplicate detection에 쓰는 canonical bin count hash
...
Cluster 대표 vector의 mean/variance/std는 선택된 모든 axis가 numeric인 multidimensional rows 기준으로 계산합니다. Domain 밖 numeric row는 cluster vector에는 반영될 수 있지만 row-level bin occupancy에서는 out-of-domain으로 제외됩니다. UI에는 raw row count, vector row count, row-level valid bin row count를 구분해 표시합니다.
...
- row-level summary: `binOccupancy`, `axisBinOccupancy`, `binOccupancyMeta`, `binOccupancyHash`
- vector basis: `clusterVectorRowCount`, `clusterVectorBasis`, `axisNumericCounts`
...
중복 방지는 fingerprint 정책을 유지하되 `binOccupancyHash`를 포함합니다. 같은 goal, axis, values, rowCount, summaryMethod라도 row-level bin occupancy가 다르면 다른 cluster로 저장됩니다.
...
- 카드에는 vector row count도 표시해 mean/variance/std 계산 기준을 row-level valid bin count와 구분합니다.
...
화면은 상단 quick navigation이 아니라 실제 탭 UI로 동작합니다. 탭을 누르면 해당 view만 표시되고 스크롤 anchor 이동이나 설정 modal 진입 없이 화면이 전환됩니다.

- 데이터 분석: CSV mapping / 분석
- Batch 업로드: Preview / 선택 저장
- 저장된 군집: 카드 grid / 필터
- 리포트: Confidence / Export
- 실험 목적 / 관리: Goal / Axis / Audit

기존 상단 오른쪽 설정/감사 버튼과 중복 설정 버튼은 제거되었고, Goal editor와 Cluster Audit 요약은 `실험 목적 / 관리` 탭으로 통합되었습니다. 저장된 군집은 responsive card grid로 표시됩니다.
...
Report UI의 axis별 coverage/equitability distribution은 `axisBinOccupancy`가 있으면 row-level basis로 표시하고, legacy cluster만 있어 axis bin summary가 없으면 `cluster vector fallback`으로 표시합니다.
